...
button_call = pygame.transform.scale(pygame.image.load(current_dir+'/assets/buttons/game_actions/call.png'), (screen_width * 0.1, screen_height * 0.08))
button_call_rect = button_call.get_rect(topleft=(screen_width * 0.708, screen_height * 0.84))
...
def update_raise_amount(raise_int):
    ...
    raise_text = pot_font.render("$" + str(raise_int), True, text_color)
    return raise_text

def draw_call_button():
    screen.blit(button_call, (screen_width * 0.708, screen_height * 0.84))
...
# drawing raise int onto screen atm

def draw_raise_state(raise_amount):
    ...
    screen.blit(raise_amount_surface, (screen_width * 0.42, screen_height * 0.79))
    pygame.display.update()
...
def main():
    ...
    title_state, settings_state, state1, state2, state3, state4, state5, state6, player_raise_state = False, False, False, False, False, False, False, False, False
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:  # Left mouse button
                    ...
                    # In-game Buttons
                    elif button_check_rect.collidepoint(event.pos) and 0 < game_state < 5 and player_raise_state == False: # Check button
                        click_sound_game_action.play()
                        player_action = "check"


                    elif button_raise_rect.collidepoint(event.pos) and 0 < game_state < 5 and player_raise_state == False: # Raise button
                        click_sound_game_action.play()
                        player_raise_state = True
                        temp_raise_amount = raise_amount
                        draw_raise_state(raise_amount)


                    elif button_fold_rect.collidepoint(event.pos) and 0 < game_state < 5 and player_raise_state == False: # Fold button
                        click_sound_game_action.play()
                        player_action = "fold"

                    elif button_call_rect.collidepoint(event.pos) and 0 < game_state < 5 and player_raise_state == False: # Call button
                        click_sound_game_action.play()
                        player_action = "call"
                    ...
                    # Raise Buttons
                        ...
                        draw_raise_state(temp_raise_amount)
                        ...
                        click_sound_game_action.play()
                        if temp_raise_amount > raise_amount + 100:
                            temp_raise_amount -= 100
                        draw_raise_state(temp_raise_amount)
        ...
        if game_state == 1:
            ...
            '''       
            # Player is Big Blind
            if player_action != None and player_blind_status == 0:
                
                player_action = None

                # Player Checks as Big Blind
                if player_action == "check":
                    raise_state = [False]
                    ai_move = ai_action(ai_bot, 1, deck[2:4], raise_state, little_blind)

                    if ai_move[0] == "check":
                        game_state += 1
                        break

                    if ai_move[0] == "raise": #AI CAN KEEP RASISING SO NEED TO DO IN LOOP
                        raise_amount = ai_move[0] + 100
                        while ai_move[0] == "raise" or player_action == "raise":
                            draw_call_button()
                            if player_action != None and player_action == "fold":
                                player_action = None
                                winner = "AI Wins"
                                drawState6(winner)
                                ...
                                player_action = None
                                player.chips -= ai_move[1]
                                pot += (ai_move[1] * 2)
                                game_state += 1
                                break
                            elif player_action == "raise":
                                player_action = None
                                ai_move = ai_action(ai_bot, 1, deck[2:4], True, little_blind)

                    if ai_move == "fold": # THIS SHOULD NEVER BE THE CASE
                        raise Exception("AI folded on a player check?")


                # Player raises as Big Blind
                elif player_action == "raise":
                    raise_state = [True, raise_amount]
                    ai_move = ai_action(ai_bot, 1, deck[2:4], raise_state, little_blind)
                    ...
                    if ai_move[0] == "raise": #AI CAN KEEP RASISING SO NEED TO DO IN LOOP
                        while ai_move[0] == "raise" or player_action == "raise":
                            ...
                            if player_action != None and player_action == "fold":
                                ...
                                player_action = None
                                ai_move = ai_move = ai_action(ai_bot, 1, deck[2:4], True, little_blind)
            ...
            else: # AI is big blind and will bet first
                ai_action(ai_bot, 1, deck[2:4], raise_state, little_blind)
'''
